...
---------------------------
# 数组Array?Ruby中的数组无类型且可变的?数组里的元素不必属于同一个
# 类型?而且其大小可变?下标越界不会抛出异常只返回nil。
...
---------------------------
# 在一个标识符或字符串的前面加上冒号的方式表示一个符号字面量。
# 但我们着重于将字符串作为一个独一无二的标识符?而不在于其内容的时候?使用符号
...
---------------------------
# 在反射代码里?符号常常被用于引用方法名。
o.respond_to? :each    # 判断该对?是否包含each方法
---------------------------
name = :size
...
---------------------------
# 对?标识 Object Identity
 a="helo"
 puts a.__id__    #返回对?在该生命周期的唯一的ID
 puts a.object_id #等同上面方法?返回对?在该生命周期的唯一的ID
 puts a.hash      #也可返回一个对?的hash码?不同与上面两个方法的值
---------------------------
# 确定一个对?所属的类
...
---------------------------
# is_a?方法和kind_of?方法判断该对?是否是某个类的某个子类的实例。
...
---------------------------
# class类定义了===操作符?可以替换is_a?方法
# 注意?这样的可读性不好?不推荐。
Numeric === x            # true: x is_a Numeric 
---------------------------
# 使用respond_to?方法判断是否可从一个对?上调用某些方法
o.respond_to? :"<<"  # true if o has an << operator
---------------------------
# 该方法的缺点在于仅仅检查类一个方法的名称?而没有检查该方法的参数。
# 该实例的解决方法是?示地将Numeric对?排除
...
---------------------------
# Ruby中的==用于值判断?而equal?方法用于判断是否同一个对?
# 这正与Java中?反。
